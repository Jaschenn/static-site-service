...
极简静态 HTML 托管服务，部署在 `static.jaschen.life`。用户通过 API Key 认证后发布 HTML 文件，获得 8 字符短链接（如 `https://static.jaschen.life/AbCdEf12`）供公开访问。提供 REST API、Web 管理面板和 CLI 工具。
...
### 依赖
所有依赖在 `app/requirements.txt`（fastapi、uvicorn、jinja2、aiosqlite、python-multipart）。无测试框架、linter 或 pyproject.toml。
...
│   ├── api_sites.py # POST /api/sites（发布）, GET /api/sites（列表）, DELETE /api/sites/{code}（删除）
│   ├── serve.py     # GET /{shortcode}（提供 HTML）, GET /404
│   └── web.py       # Web UI：首页、/verify、/login、/dashboard、/logout、/delete/{code}、/cli
├── security.py      # SecurityHeadersMiddleware（X-Content-Type-Options, X-Frame-Options, Referrer-Policy, Permissions-Policy）
├── ratelimit.py     # 内存速率限制器（IP 级 + key 级），用于 API 端点
├── services/
│   ├── email.py     # 验证码生成 + SMTP 发送（MOCK_EMAIL=true 时打印到 stdout）
│   └── shortcode.py # 8 字符密码学随机短码生成
├── templates/       # Jinja2 模板（base.html, index.html, login.html, verify.html, dashboard.html）
...
- **sites**: `shortcode TEXT PK`（8 char alphanumeric）, `email FK`, `title`, `size_bytes`, `created_at`

### 配置（通过 .env）
关键默认值：`DATABASE_PATH=/data/data.db`, `SITES_DIR=/data/sites`, `MAX_UPLOAD_SIZE=4MB`, `MOCK_EMAIL=true`, `TOKEN_EXPIRY=600`（10 分钟）, `SESSION_SECRET=change-me-in-production`
...
`SecurityHeadersMiddleware` 全局注入 `X-Content-Type-Options: nosniff`、`X-Frame-Options: SAMEORIGIN`、`Referrer-Policy: strict-origin-when-cross-origin`。用户页面（`serve.py`）额外返回严格的 `Content-Security-Policy` 头，禁止脚本和框架嵌入。

### 验证码/API Key 逻辑在 routes 中存在重复
`api_keys.py` 和 `web.py` 中的验证码兑换流程相似但独立实现。如需修改这个流程，两处都需要更新。

### 无测试
项目目前没有任何测试、linting 或 CI。添加新功能时建议先补充关键路径的测试。
